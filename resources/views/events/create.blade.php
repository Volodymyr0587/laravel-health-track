    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4">
            ...
            <x-forms.form action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <x-forms.input label="Event Name" name="name" value="A visit to the family doctor" />
                <x-forms.input label="Event Location" name="location" value="Volyn Regional Clinical Hospital, ave. Presidenta Hrushevskyi, 21
                Lutsk, Volyn region, 43000" />
                <x-forms.input label="Event Date and Time" name="event_time" type="datetime-local" />
                <x-forms.input label="Event Description" name="description"  />
                <x-forms.input label="Event Price (if free, leave the field empty)" name="price" value="750.00" />
